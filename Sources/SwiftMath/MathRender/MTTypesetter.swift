...
func getInterElementSpaceArrayIndexForType(_ type:MTMathAtomType, row:Bool) -> Int {
    switch type {
        ...
        //By Alpha
        case .color, .textcolor, .colorBox, .ordinary, .placeholder, .accent:   // A placeholder is
            return 0
        //By Alpha
        case .largeOperator:
            return 1
...
public class MTTypesetter {//By Alpha
    ...
    public static func createLineForMathList(_ mathList:MTMathList?, font:MTFont?, style:MTLineStyle, finalized:Bool = false) -> MTMathListDisplay? { //By Alpha
        //let finalizedList = mathList?.finalized
        //By Alpha
        var finalizedList:MTMathList = MTMathList()
        if let mathList = mathList {
            if finalized {
                finalizedList = mathList
            } else {
...
//                    display!.localTextColor = MTColor(fromHexString: colorAtom.colorString)
                ...
                case .radical:
                    ...
                    if rad.degree != nil {
                        ...
                        let degree = MTTypesetter.createLineForMathList(rad.degree, font:font, style:.scriptOfScript, cramped: false)
                        //By Alpha
    ...
    func getRadicalGlyphWithHeight(_ radicalHeight:CGFloat) -> MTDisplayDS? {
        ...
        if glyphAscent + glyphDescent < radicalHeight {
            ...
            glyphDisplay!.extenderPosition = CGPoint(x: glyphWidth - deltaX*0.1, y: deltaY*1.1)
            //By Alpha
    ...
    func makeAccent(_ accent:MTAccent?) -> MTDisplay? {
        ...
        let accentGlyph = self.findGlyphForCharacterAtIndex(end, inString:accent!.nucleus)
...
//        let accenteeAdjustment = styleFont.mathTable!.getTopAccentAdjustment(accenteeGlyph)
        if glyphWidth < accenteeWidth*0.85 && !isSingleCharAccentee(accent){
            if let display = constructGlyph(accentGlyph, withWidth: accenteeWidth*0.85) {
                ...
                glyphAscent = display.ascent
                glyphDescent = display.descent
        ...
        for row in 0..<(table?.numRows ?? 0) {
            if let display = (tableDisplay.subDisplays[row] as? MTMathListDisplay) {
                ...
                for col in 0..<(table?.numColumns ?? 0) {
                    ...
                    //table?.cells[row][col].display.position.x += x
                }
            }
